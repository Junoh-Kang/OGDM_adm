...
class Dataset_(Dataset):
    def __init__(self, data_dir, num=-1):
        self.data_dir = pathlib.Path(data_dir)
        self.files = sorted([file for ext in IMAGE_EXTENSIONS
                             for file in self.data_dir.glob('*.{}'.format(ext))])
        if num > 0:
            # self.files = random.choices(self.files,k=N)
            self.files = self.files[:num]
        self.trsf_list = [transforms.PILToTensor()]
        self.transforms = transforms.Compose(self.trsf_list)
...
def prepare_evaluation():
    ...
    ###
    parser.add_argument("--real_moment_path", type=str, default=None, help="specify the directory of the folder to save real feat.")
    parser.add_argument("--real_feat_path", type=str, default=None, help="specify the directory of the folder to save real feat.")
    parser.add_argument("--gen_feat_path", type=str, default=None, help="specify the directory of the folder to save real feat.")
    parser.add_argument("--dset2_feats", type=str, default=None, help="specify the path of *.npy that contains features of dset1 (real). \
                        If not specified, StudioGAN will automatically extract feat2 using the whole dset2.")
    ###
    ...
    parser.add_argument("--compute", "-compute", type=bool, default=True)
...
def evaluate(local_rank, args, world_size, gpus_per_node):
    ...
    load_dset1 = ("prdc" in args.eval_metrics and args.dset1_feats == None)
    
    if load_dset1:
        dset1 = Dataset_(data_dir=args.dset1, num=args.N)
        if local_rank == 0:
            print("Size of dset1: {dataset_size}".format(dataset_size=len(dset1)))
    
    if args.dset2_feats is None:
        dset2 = Dataset_(data_dir=args.dset2, num=args.N)
        if local_rank == 0:
    ...
    if load_dset1:
        ...
        if local_rank == 0:
            print ("Save real_features to disk.")
            
            np.savez(args.real_feat_path, **{"real_feats": dset1_feats.detach().cpu(),
                                   "real_probs": dset1_probs.detach().cpu(),
                                   "real_labels": dset1_labels})
    ...
    if "fid" in args.eval_metrics:
        if args.dset1_moments is None:
            ...
            if args.real_moment_path is not None:
                np.savez(args.real_moment_path, **{"mu": mu1,
                                   "sigma": sigma1})
    ...
    if "prdc" in args.eval_metrics:
        ...
        if args.dset1_feats is None:
            if args.N < 0:
                args.N = len(dset1)
            dset1_feats_np = np.array(dset1_feats.detach().cpu().numpy(), dtype=np.float64)[:args.N]
            dset1_mode = "dset1"
        else:
            if args.N < 0:
                args.N = len(dset1)
            dset1_feats_np = np.load(args.dset1_feats, mmap_mode='r')["real_feats"][:args.N]
            dset1_mode = "pre-calculated dset1_feats"
        if args.N < 0:
            args.N = len(dset2)
        if args.dset2_feats is None:
            dset2_feats_np = np.array(dset2_feats.detach().cpu().numpy(), dtype=np.float64)
        else:
            dset2_feats_np = np.load(args.dset2_feats, mmap_mode='r')["real_feats"]
            dset2_mode = "pre-calculated dset1_feats"
            if args.seed > 0:
                np.random.seed(args.seed)
                np.random.shuffle(dset2_feats_np)
        dset2_feats_np = dset2_feats_np[:args.N]

        if args.gen_feat_path is not None:
            np.savez(args.gen_feat_path, **{"real_feats": dset2_feats.detach().cpu(),
                                   "real_probs": dset2_probs.detach().cpu(),
                                   "real_labels": dset2_labels})
            if not args.compute:
                return
        print ("start")
        ...
        if local_rank == 0:
            ...
            print ("P R D C : {} {} {} {}".format(prc, rec, dns, cvg))
